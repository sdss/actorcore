...
bossExposureId = {'exposureId': [1234500],
                  'BeginExposure': [1411421207, 56922.89]}
...
tccState['stopped'] = merge_dicts(tccBase, axisStatStopped, atStow,
                                  axisCmdState_halted, noErrorCode)
tccState['halted'] = merge_dicts(tccBase, axisStatClear, atGangChange,
                                 axisCmdState_halted, haltedErrorCode)
tccState['tracking'] = merge_dicts(tccMoving, axisStatClear, atSomeField,
                                   axisCmdState_tracking, noErrorCode)
tccState['slewing'] = merge_dicts(tccMoving, axisStatClear, atSomeField,
                                  axisCmdState_slewing, noErrorCode)
tccState['bad'] = merge_dicts(tccBase, axisStatBad, atInstChange,
                              axisCmdState_halted, haltedErrorCode)
tccState['halted_low'] = merge_dicts(tccBase, axisStatBadAz, atAlt15,
                                     axisCmdState_halted, haltedOneErrorCode)
tccState['badAz'] = merge_dicts(tccBase, axisStatBadAz, atStow,
                                axisCmdState_azhalted, haltedOneErrorCode)
...
guiderState['apogeemangaNoneLoaded'] = merge_dicts(guiderOff, apogeemangaLoadedNoSurveyMode,
                                                   mangaC)
...
guiderState['apogeemangaGlobularLoaded'] = merge_dicts(guiderOff, apogeemangaGlobularLoaded,
                                                       mangaC)
...
mangaDitherPointing = {
    'pointingInfo': [3000, 2, 'A', 30., 40., 3., 2., 5500, 'MaNGA', 'MaNGA dither']
}
mangaGlobularPointing = {
    'pointingInfo': [3000, 2, 'A', 30., 40., 3., 2., 5500, 'MaNGA', 'MaNGA Globular']
}
manga10Pointing = {'pointingInfo': [3001, 2, 'A', 30., 40., 3., 2., 5500, 'MaNGA', 'MaNGA 10min']}
mangaStarePointing = {
    'pointingInfo': [3001, 2, 'A', 30., 40., 3., 2., 5500, 'MaNGA', 'MaNGA Stare']
}
MaStarPointing = {'pointingInfo': [3001, 2, 'A', 30., 40., 3., 2., 5500, 'MaNGA', 'MaStar']}
apogeeLeadPointing = {
    'pointingInfo': [4000, 3, 'A', 40., 50., 4., 3., 10500, 'APOGEE2&MANGA', 'APOGEE lead']
}
apogeeLeadPointingCart7 = {
    'pointingInfo': [4000, 7, 'A', 40., 50., 4., 3., 10500, 'APOGEE2&MANGA', 'APOGEE lead']
}
apgoeemangaDitherPointing = {
    'pointingInfo': [5000, 4, 'A', 50., 60., 5., 4., 5500, 'APOGEE-2&MaNGA', 'MaNGA dither']
}
apgoeemangaGlobularPointing = {
    'pointingInfo': [5000, 4, 'A', 50., 60., 5., 4., 5500, 'APOGEE-2&MaNGA', 'MaNGA Globular']
}
apgoeemanga10Pointing = {
    'pointingInfo': [5000, 4, 'A', 50., 60., 5., 4., 5500, 'APOGEE-2&MaNGA', 'MaNGA 10min']
}
apogeemangaStarePointing = {
    'pointingInfo': [5001, 4, 'A', 50., 60., 5., 4., 5500, 'APOGEE-2&MaNGA', 'MaNGA Stare']
}
apogeemangaMaStarPointing = {
    'pointingInfo': [5001, 4, 'A', 50., 60., 5., 4., 5500, 'APOGEE-2&MaNGA', 'MaStar']
}
...
platedbState = {}
platedbState['boss'] = merge_dicts(bossPointing, apogeeDesignNone, BOSSInstrumentPlugged)
platedbState['apogee'] = merge_dicts(apogeePointing, apogeeDesignNone, APOGEEInstrumentPlugged)
platedbState['mangaDither'] = merge_dicts(mangaDitherPointing, apogeeDesignNone,
                                          BOSSInstrumentPlugged)
...
platedbState['mangaStare'] = merge_dicts(mangaStarePointing, apogeeDesignNone,
                                         BOSSInstrumentPlugged)
platedbState['mangaGlobular'] = merge_dicts(mangaGlobularPointing, apogeeDesignNone_manga_short,
                                            BOSSInstrumentPlugged)
platedbState['MaStar'] = merge_dicts(MaStarPointing, apogeeDesignNone, BOSSInstrumentPlugged)
platedbState['MaStar_short'] = merge_dicts(MaStarPointing, apogeeDesignNone_manga_short,
                                           BOSSInstrumentPlugged)
platedbState['MaStar_coobs_short'] = merge_dicts(MaStarPointing, apogeeDesignNone_manga_short,
                                                 BothInstrumentsPlugged)
platedbState['apgoeemangaDither'] = merge_dicts(apgoeemangaDitherPointing, apogeeDesignNone,
                                                BothInstrumentsPlugged)
platedbState['apogeemangaGlobular'] = merge_dicts(apgoeemangaGlobularPointing,
                                                  apogeeDesignNone_manga_short,
                                                  BothInstrumentsPlugged)
platedbState['apgoeemanga10'] = merge_dicts(apgoeemanga10Pointing, apogeeDesignNone,
                                            BothInstrumentsPlugged)
...
platedbState['apogeeLead'] = merge_dicts(apogeeLeadPointing, apogeeDesignNone,
                                         BothInstrumentsPlugged)
platedbState['apogeeLeadCart7'] = merge_dicts(apogeeLeadPointingCart7, apogeeDesignNone,
                                              BothInstrumentsPlugged)
platedbState['apogeeLead1000s'] = merge_dicts(apogeeLeadPointing, apogeeDesign1000,
                                              BothInstrumentsPlugged)

platedbState['apogeeLead1000sCart7'] = merge_dicts(apogeeLeadPointingCart7, apogeeDesign1000,
                                                   BothInstrumentsPlugged)
...
# sop state setup
bypasses = [
    'ffs', 'lamp_ff', 'lamp_hgcd', 'lamp_ne', 'axes', 'isBoss', 'isApogee', 'isMangaDither',
    'isMangaStare', 'isManga10', 'isMangaGlobular', 'isApogeeManga10', 'isApogeeLead',
    'isApogeeMangaDither', 'isApogeeMangaStare', 'isApogeeMangaGlobular', 'gangToCart',
    'gangToPodium', 'slewToField', 'guiderDark', 'guider_decenter', 'isMaStar',
    'isApogeeMangaMaStar'
]
sopNoBypass = {'bypassNames': bypasses, 'bypassedNames': []}
sopEmptyCommands = {
    'surveyCommands': ('gotoField', 'gotoStow', 'gotoInstrumentChange', 'gotoAll60', 'gotoStow60'),
    'survey': ['UNKNOWN', 'None']
}
sopBossCommands = {
    'surveyCommands': ('gotoField', 'gotoStow', 'gotoInstrumentChange', 'gotoAll60', 'gotoStow60',
...
sopApogeeCommands = {
    'surveyCommands':
    ('gotoField', 'gotoStow', 'gotoInstrumentChange', 'gotoAll60', 'gotoStow60', 'doApogeeScience',
     'doApogeeSkyFlats', 'gotoGangChange', 'doApogeeDomeFlat'),
    'survey': ['APOGEE', 'None']
}
sopApogeeMangaCommands = {
    'surveyCommands': ('gotoField', 'gotoStow', 'gotoInstrumentChange', 'gotoAll60', 'gotoStow60',
                       'doBossCalibs', 'doApogeeMangaDither', 'doApogeeMangaSequence',
                       'doApogeeSkyFlats', 'gotoGangChange', 'doApogeeDomeFlat'),
    'survey': ['APOGEE-2&MaNGA', 'APOGEE lead']
}
sopState = {}
...
class ActorTester(object):
    ...
    def setUp(self):
        ...
        if getattr(self, 'actor', None) is None:
            ...
            self.actor = FakeActor.newActor(self.name,
                                            productName=productName,
                                            cmd=self.cmd,
                                            attachCmdSets=self.attachCmdSets)

        self.actorState = ActorState(cmd=self.cmd,
                                     models=list(models.keys()),
                                     modelParams=list(models.values()))
