...
def lcoTCCCards(models, cmd=None):
    ...
    try:
        ...
        for i in range(len(orientNames)):
            cards.append(makeCard(cmd, 'M2' + orientNames[i], float(secOrient[i]),
                                  'TCC SecOrient'))
    except Exception as e:
    ...
    try:
        ...
        for i in range(len(orientNames)):
            cards.append(makeCard(cmd,
                                  'M2' + orientNames[i], float(secOrient[i]),
                                  'TCC SecOrient'))
    except Exception as e:
        cmd.warn("text='failed to generate the SecOrient cards: %s'" % (e))
...
def plateCards(models, cmd):
    ...
    if surveyMode is not None and survey != 'None':
        cards.append(
            makeCard(cmd, 'SRVYMODE', surveyMode, 'Survey leading this observation and its mode'))
...
def writeFits(cmd,
    ...
    try:
        if cmd is not None:
            cmd.inform('text="writing %sFITS files for %s (%d threads)"' %
                       (caller, filename, threading.active_count()))
        else:
            logging.info('writing %sFITS files for %s (%d threads)' %
                         (caller, filename, threading.active_count()))
        ...
        tempFile = tempfile.NamedTemporaryFile(dir=directory,
                                               mode='wb',
                                               suffix=suffix,
                                               prefix=filename + '.',
                                               delete=False)
        ...
        if doCompress:
            ...
            if filename[-3:] != '.gz':
                outName += '.gz'
            tempFile = gzip.GzipFile(fileobj=tempFile,
                                     filename=filename,
                                     mode='wb',
                                     compresslevel=4)
        else:
